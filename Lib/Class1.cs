...
namespace Lib
{
    public class Class1
    {
        ...
        public static void Replacing(string[,] source, int sourceRow, string[,] target, int targetRow)
        {
            for(int a = 0; a < 11; a++)
            {
                target[targetRow, a] = source[sourceRow, a];
            }
        }
        ...
        public static void YouCarRepositioning(int[,] coords, string[,] level0)
        {
            ...
            level0[coords[1, 2], coords[1, 3]] = "C";
        ...
        public static string IfGameIsOver(int[,] coords, string[,] level)
        {
            bool AreYouAlive = level[coords[0, 0] - 1, coords[0, 1]] == " " &&  level[coords[0, 2] - 1, coords[0, 3]] == " " && level[coords[0, 4] - 1, coords[0, 5]] == " ";
            if(AreYouAlive == true)
            { return ""; } 
            else
            {
